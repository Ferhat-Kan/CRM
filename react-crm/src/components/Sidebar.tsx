...
export default function Sidebar(props: any) {
  ...
  const navIcons = (text: any, screen: any): React.ReactNode => {
    switch (text) {
      ...
      case 'admin':
        return screen === 'admin' ? <FaTools fill="#3e79f7" /> : <FaTools />;
      default:
              ...
              {navList.map((text, index) => (
                <ListItem key={text} disablePadding>
                  <StyledListItemButton
                    onClick={() => {
                      const path =
                        text === 'admin' ? '/app/admin/users' : `/app/${text}`;
                      navigate(path);
                      setScreen(text);
                    }}
                    selected={screen === text}
                  >
                    <ListItemIcon>{navIcons(text, screen)}</ListItemIcon>
                    <StyledListItemText
                      primary={text === 'admin' ? 'Admin Panel' : text}
